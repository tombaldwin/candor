...
| `tsconfig.json`, `package.json` + `.ts` sources | TypeScript | [candor-ts/AGENTS.md](https://github.com/tombaldwin/candor-ts/blob/main/AGENTS.md) — clone + `node scan.mjs .` (not yet on npm: clone HEAD, check `candor.spec` = 0.4 in the envelope) |
