...
- **Five conformance `Engine` copies** (`gen_completeness`, `gen_fs_kind`, `gen_masking`, `gen_netclass`,
  ...
  **MEASURED 2026-09-12, and the entry understates it in one way and overstates it in another.**
  The `ENGINES = [("rust", scan_rust), …]` line is byte-identical in four of the five (sha `6d35549032d3`),
  so that part is pure copy. The real duplication is one level down: `_rust_tree`/`_java_tree`/`_ts_tree`
  each exist in THREE of these files in THREE variants. But the variance is not rot — it is the crate
  name, the `GENERATED by <gen>.py` header, whether the body arrives as `body` or `case["rust"]`, and
  whether `_write_config` is called. So this is a parameterised helper waiting to be extracted
  (`(name, body, hook)`), not a dedup, and "should import gen_differential" is not on its own the fix.
  **DO NOT DO THIS WHILE A STAT/MASKING WAVE IS IN FLIGHT:** `gen_stat_locator.py` imports
  `gen_masking` and calls `gm._rust_tree`/`_java_tree`/`_ts_tree` directly, so changing those signatures
  breaks the instrument that the ⟨0.37⟩ engine work is measured by. Land it after ⟨0.37⟩, and take
  `gen_stat_locator` with it. R288: an instrument with three copies has no owner.
...
(71)` of 207 sources — fixtures and samples, i.e. the norm, not a broken build; the field case 93 of 2237. A
...
a genuine violation still DOMINATES (the field case under `deny Exec` exits 1, not 2 — the rung converts false
...
parse and alone is ~72% of the field case's analysis, and it runs in a plain `for (ClassNode cn : classes)` loop.
...
- **`[P1]` A CHEAP REPORT REFRESH — BUILT 2026-08-21, candor-java, opt-in `CANDOR_REFRESH=<dir>`.
  ...
  **The codec refuses unknown field and value types rather than skipping them**, and a refusal abandons
  ...
  on commons-lang3, gson, jackson-core, joda-time, sqlite-jdbc and the field case, 6 controls armed.
  ...
  **WHERE IT LANDED** (the field case, 2,602 classes):
...
- **`[P1 — ORIGINAL FILING]` A CHEAP REPORT REFRESH — the edit-time loop re-analyses everything on every turn.** Field
  report from the field case (candor-java 0.26.0, 2,259 class files / 15MB of bytecode, a 4.9MB baseline),
...
- **[OPEN — P3, and the measurement REFRAMED it: blocked on value provenance, not on a table] An
  ...
  **So measure BEFORE designing, and be aware nothing below is measured yet** — there is no Android SDK on
  this machine and no Android corpus (the field case is a Tomcat webapp; `local.properties` is a plain Gradle
...
- **[CLOSED 2026-08-03 — all four batches; see the body for what the source refuted] Ledger-mined classifier breadth** (data from the 2026-07-14 four-ecosystem sweep):
  ...
  **THE DURABLE DISTINCTION, needed three times in one batch:** a library moving bytes through a handle
  ...
  (S3 → Net/Fs, SES → Net; the field case's top uncovered) + commons-io → Fs; swift — the uncovered list is
