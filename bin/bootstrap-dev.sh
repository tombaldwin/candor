...
if [ "$WITH_CORPORA" = 1 ]; then
  ...
  [ -d "$ROOT/the field case/.git" ] || git clone -q git@bitbucket.org:<redacted org>/the field case.git "$ROOT/the field case" || warn "the field case clone failed (needs Bitbucket auth; optional)"
