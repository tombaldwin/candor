...
| File | Role |
|------|------|
| `candor-review.sh` | The review for the **JVM** (candor-java reads bytecode). Scan compiled classes → diff vs baseline → gate. Standalone (CI / manual / agent) or called by the hook. Prints the gained effects, their blast radius, and any `AS-EFF-…` policy violation; exit 1 if any of those fire — **including a fail-closed `incomplete` verdict** (part of the target unread/unjudged, SPEC ⟨0.21⟩/⟨0.28⟩/⟨0.30⟩), which is a violation could-be-hiding-here signal, not a safe-to-ignore setup error. |
